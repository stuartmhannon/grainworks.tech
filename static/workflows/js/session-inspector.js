...
import { relativeTime, escapeHtml, fetchJSON } from './shared-utils.js';
...
function buildTimeline(sessions) {
  ...
  // Cap visible sessions at 50, show more link
  const SHOW_MAX = 50;
  const showSessions = sessions.slice(0, SHOW_MAX);

  showSessions.forEach(s => {
    list.appendChild(buildSessionEntry(s));
  });

  if (sessions.length > SHOW_MAX) {
    const moreEl = document.createElement('div');
    moreEl.className = 'si-show-more';
    moreEl.style.cssText = 'text-align:center;padding:12px;cursor:pointer;color:var(--accent);font-size:11px;';
    moreEl.textContent = `Show all ${sessions.length} sessions ▾`;
    moreEl.addEventListener('click', function () {
      this.remove();
      sessions.slice(SHOW_MAX).forEach(s => {
        list.appendChild(buildSessionEntry(s));
      });
    });
...
export async function renderSessionInspector(container) {
  try {
    ...
    // Initial fetch — fetchAndRender shows spinner before the request
    ...
    console.error('renderSessionInspector failed:', err);
    if (container) {
      container.innerHTML = '<div class="error-fallback"><span class="error-fallback-icon">⚠</span>Something went wrong loading the session inspector.</div>';
    }
  }
}
...
async function fetchAndRender() {
  ...
  // Show loading spinner immediately before the request
  root.innerHTML = '<div class="inbox-loading"><div class="inbox-spinner"></div>Loading sessions…</div>';
  ...
  try {
    ...
    if (root) {
      root.innerHTML = '<div class="error-fallback"><span class="error-fallback-icon">⚠</span>Session data unavailable.</div>';
    }
  }
}
